...
public enum IotDeviceState {
	...
	IotDeviceState(Integer index, String name, String color, Boolean deletable) {
		this.index = index;
		this.name = name;
		this.color = color;
	...
	public static IotDeviceState getByIndex(Integer index) {
		for (IotDeviceState c : IotDeviceState.values()) {
			if (c.getIndex() == index) {
				return c;
			}
		}
